...
namespace EntityFrameworkProject.Areas.AdminArea.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        //        {
        //            if (!productUpdateVM.Photo.CheckFileType("image/"))
        //            {
        //                ModelState.AddModelError("Photo", "Please choose correct image type");
        //                return View();
        //            }

        //            if (!productUpdateVM.Photo.CheckFileSize(500))
        //            {
        //                ModelState.AddModelError("Photo", "Please choose correct image size");
        //                return View();
        //            }

        //            string fileName = Guid.NewGuid().ToString() + "_" + productUpdateVM.Photos.FillName;

        //            Product dbProduct = await _context.Products.AsNoTracking().FirstOrDefaultAsync(m => m.Id == id);

        //            if (dbProduct is null) return NotFound();

        //            if (dbProduct.Title.Trim().ToLower() == dbProduct.Title.Trim().ToLower()
        //                && dbProduct.Description.Trim().ToLower() == dbProduct.Description.Trim().ToLower()
        //                && dbProduct.Photo == dbProduct.Photo)
        //            {
        //                return RedirectToAction(nameof(Index));
        //            }
        ...
        //            {
        //                await dbProduct.Photo.CopyToAsync(stream);
        //            }

        //            dbProduct.Image = fileName;

        //            _context.SliderDetails.Update(dbProduct);

        //            await _context.SaveChangesAsync();

        //            string dbPath = Helper.GetFilePath(_env.WebRootPath, "img", dbProduct.Image);
